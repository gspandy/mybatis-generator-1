...
public class TableConfiguration extends PropertyHolder {
    ...
    public TableConfiguration(Context context) {
        ...
        selectSelectiveStatementEnabled = false;
    ...
    public void setSelectSelectiveStatementEnabled(
            boolean selectSelectiveStatementEnabled) {
    ...
    public XmlElement toXmlElement() {
        ...
        if (!selectSelectiveStatementEnabled) {
            xmlElement.addAttribute(new Attribute(
                    "enableSelectSelective", "false")); //$NON-NLS-1$ //$NON-NLS-2$
        }
