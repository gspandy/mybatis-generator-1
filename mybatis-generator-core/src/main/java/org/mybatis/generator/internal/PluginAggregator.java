...
public final class PluginAggregator implements Plugin {
    ...
    public boolean sqlMapSelectSelectiveElementGenerated(XmlElement element,
            IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.sqlMapSelectSelectiveElementGenerated(element,
                    introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean clientSelectSelectiveMethodGenerated(Method method,
            Interface interfaze, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.clientSelectSelectiveMethodGenerated(method, interfaze,
                    introspectedTable)) {
                rc = false;
                break;
            }
    ...
    public boolean clientSelectSelectiveMethodGenerated(Method method,
        ...
        for (Plugin plugin : plugins) {
            if (!plugin.clientSelectSelectiveMethodGenerated(method,
                    topLevelClass, introspectedTable)) {
                rc = false;
                break;
            }
